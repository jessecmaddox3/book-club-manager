...
def main():
 ...
 with tempfile.TemporaryDirectory(prefix='book club launcher ') as temporary:
  base=Path(temporary)
  with zipfile.ZipFile(ROOT/'artifacts'/f'book-club-manager-{version}.zip') as source:source.extractall(base)
  project=base/f'book-club-manager-{version}'
  env={key:value for key,value in os.environ.items() if not key.startswith(('BOOKCLUB_','SUPABASE_','DATABASE_','ANTHROPIC_','VERCEL','AWS_LAMBDA'))}
  env.update(PORT=str(port),BOOKCLUB_LAUNCH_NO_OPEN='1',NEXT_TELEMETRY_DISABLED='1')
  ...
  with (base/'launcher.log').open('w+',encoding='utf-8',errors='replace') as log:
   ...
   try:
    ...
    while True:
     ...
     try:
      ...
      with urllib.request.urlopen(origin+'/login',timeout=3) as response:
       assert 'Lantern Reading Room' in response.read().decode()
      break
     except OSError:
    ...
    assert (project/'node_modules/.bookclub-lock').is_file()
    assert (project/'.next/bookclub-source').is_file()
