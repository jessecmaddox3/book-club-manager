...
const env={...process.env,BOOKCLUB_MODE:'demo',NEXT_TELEMETRY_DISABLED:'1'};
...
env.PORT=String(port);env.BOOKCLUB_ORIGIN=`http://127.0.0.1:${port}`;
...
async function fingerprint(){
 ...
 for(const relative of ['src','public','db','scripts','package.json','package-lock.json','next.config.ts','postcss.config.mjs','tsconfig.json'])await visit(relative);
...
async function main(){
 ...
 child.on('message',message=>{if(message?.kind==='bookclub-ready'&&message.origin===env.BOOKCLUB_ORIGIN)started=true;});
 ...
 const ready=()=>new Promise(resolve=>{
  const request=http.get(env.BOOKCLUB_ORIGIN+'/login',response=>{response.resume();resolve(response.statusCode===200);});
 ...
 for(let attempt=0;attempt<90&&!exited;attempt++){
  if(started&&await ready()){
   console.log(`Open ${env.BOOKCLUB_ORIGIN}\nKeep this window open while using the club. Press Control+C to stop it safely.`);
   if(env.BOOKCLUB_LAUNCH_NO_OPEN!=='1'){
    const [command,args]=process.platform==='darwin'?['open',[env.BOOKCLUB_ORIGIN]]:process.platform==='win32'?['rundll32',['url.dll,FileProtocolHandler',env.BOOKCLUB_ORIGIN]]:['xdg-open',[env.BOOKCLUB_ORIGIN]];
...
main().catch(error=>{console.error(error instanceof Error?error.message:'The club could not start.');process.exitCode=1;});
