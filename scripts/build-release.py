...
def main():
 ...
 archive=output/f'book-club-manager-{version}.zip';manifest=[]
 with zipfile.ZipFile(archive,'w',compression=zipfile.ZIP_DEFLATED,compresslevel=9) as bundle:
  for name in files:
   ...
   content=source.read_bytes();info=zipfile.ZipInfo(f'book-club-manager-{version}/'+name,(2026,1,1,0,0,0));info.create_system=3
 ...
 (output/f'book-club-manager-{version}-files.json').write_text(json.dumps(manifest,indent=2)+'\n')
